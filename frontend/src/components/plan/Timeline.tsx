...
import { Edit2, Trash2, Check, Lock, Clock, Calendar, CalendarDays } from 'lucide-react';
import { formatDate, getDayName, getShortDayName, DateFormats } from '@/utils/dateUtils';
...
export function Timeline({ data, onApprove, onLock, onDelete, onEdit }: TimelineProps) {
  ...
  const [editData, setEditData] = useState<Partial<ScheduleBlock>>({});

  const handleEditStart = (block: ScheduleBlock) => {
  ...
  data.blocks.forEach(block => {
    const day = new Date(block.weekStart ?? data.weekStart).getDay();
    const dayName = days[day === 0 ? 6 : day - 1];
    if (blocksByDay[dayName]) {
      blocksByDay[dayName].push(block);
    }
  });
        ...
        {days.map((day, index) => {
          ...
          return (
            <div key={day} className="space-y-2">
              <div className={`
                text-center py-3 rounded-xl transition-all duration-200
                ${isTodayDate 
              ...
              `}>
                ...
                <div className={`text-xl font-bold ${isTodayDate ? 'text-white' : ''}`}>
                  {formatDate(date, 'd')}
                </div>
                <div className={`text-[10px] ${isTodayDate ? 'text-blue-100' : 'text-gray-400'}`}>
                  {isTodayDate ? 'Today' : formatDate(date, 'MMM')}
                </div>
              </div>
                    ...
                    {editingBlock === block.id ? (
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-500">
                          ...
                          <span className={`font-medium ${getPriorityColor(block.priority ?? 3)}`}>
                            P{block.priority ?? 3} · {getPriorityLabel(block.priority ?? 3)}
                          </span>
