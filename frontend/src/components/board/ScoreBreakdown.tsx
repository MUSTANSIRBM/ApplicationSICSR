    ...
    <div className="bg-white/50 rounded-md p-3 space-y-2">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
            <div 
              ...
              style={{ 
                width: `${Math.min((defect.severity / MAX_SEVERITY) * 100, 100)}%`,
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
            <div 
              className="h-full rounded-full transition-all duration-500"
              style={{ 
                width: `${Math.min((defect.overdueDays / MAX_OVERDUE_DAYS) * 100, 100)}%`,
                background: getColorForValue((defect.overdueDays / MAX_OVERDUE_DAYS) * 100)
              }} 
            />
          </div>
          <span className="w-16 text-right font-mono text-gray-600">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
            <div 
              className="h-full rounded-full transition-all duration-500"
              style={{ 
                width: `${(defect.trafficImpact / MAX_TRAFFIC_IMPACT) * 100}%`,
      ...
      <div className="flex items-center justify-between pt-1 border-t border-gray-200/50">
        ...
        <span 
          ...
          style={{ color: getColorForValue(total) }}
        >
          {total.toFixed(1)}
        </span>
      </div>
