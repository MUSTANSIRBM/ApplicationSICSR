...
import { Search, X, Filter, Calendar, Clock, AlertTriangle } from 'lucide-react';
...
const statuses = ['all', 'new', 'scheduled', 'deferred', 'completed'] as const;
...
export function DefectList({ 
  ...
  const handleEditStart = (defect: Defect) => {
    ...
    setEditData({
      ...
      tier: defect.tier,
      impactScore: defect.impactScore
    });
  };
          ...
          return (
            <div key={tier} className="contents">
              ...
              <div className="col-span-full flex items-center gap-2 pt-2 pb-1">
                <Badge variant={tier as any} size="md">
                  {getTierDisplay(tier)}
                </Badge>
              ...
              {items.map(defect => (
                <div key={defect.id} className="animate-fade-in">
                  {editingId === defect.id ? (
                    <div className="bg-white rounded-xl shadow-lg border border-blue-300 p-4 sm:p-5">
